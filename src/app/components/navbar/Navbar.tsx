...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    // El flag "passive: true" mejora el rendimiento del scroll
  ...
  return (
    <header
      className="fixed top-0 inset-x-0 z-50 flex justify-center p-0 sm:p-4 pointer-events-none"
      ref={menuRef}
    >
      <nav
      ...
      >
        ...
        >
          ...
          <div className="relative w-4 h-4 flex flex-col justify-between items-center">
            <span
              className={`absolute h-0.5 w-full bg-current rounded-full transition-all duration-300 ${isMenuOpen ? "rotate-45 top-1.5" : "top-0"
                }`}
            />
            <span
              className={`absolute h-0.5 w-full bg-current rounded-full transition-all duration-200 top-1.5 ${isMenuOpen ? "opacity-0 scale-0" : "opacity-100"
                }`}
            />
            <span
              className={`absolute h-0.5 w-full bg-current rounded-full transition-all duration-300 ${isMenuOpen ? "-rotate-45 top-1.5" : "top-3"
                }`}
            />
          </div>
        </button>
      ...
      {/* Menú móvil con ID para accesibilidad */}
      <div
        id="mobile-menu"
      ...
      >
        <div className="flex flex-col p-2 space-y-1 overflow-hidden border rounded-[24px] bg-[#0B1120]/90 backdrop-blur-xl border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.6)]">
          {pathname !== "/login" && (
            <Link
              href="/login"
              className="block w-full px-4 py-3 text-sm font-medium text-left text-gray-300 transition-colors rounded-[16px] hover:bg-white/5 hover:text-white active:bg-white/10"
            >
              Iniciar sesión
            </Link>
          )}
          {pathname !== "/register" && (
            <Link
              href="/register"
              className="block w-full px-4 py-3 text-sm font-medium text-center text-white transition-all bg-gradient-to-r from-[#FF8C33] to-[#FF7400] rounded-[16px] shadow-lg shadow-orange-500/10 active:scale-[0.99]"
            >
              Registrarse
            </Link>
          )}
