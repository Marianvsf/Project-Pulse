...
const faqEntries: FAQEntry[] = [
  { id: 1, question: "¿Cómo funciona la seguridad y privacidad?", answer: "Usamos cifrado AES-256, backups automáticos diarios y controles de acceso por rol. Tus datos nunca se comparten con terceros.", keywords: ["seguridad", "privacidad", "cifrado", "datos", "proteccion"] },
  { id: 2, question: "¿Puedo cancelar mi plan en cualquier momento?", answer: "Sí. Puedes cancelar en cualquier momento desde tu cuenta, sin penalizaciones ni cargos adicionales.", keywords: ["cancelar", "cancelacion", "baja", "plan"] },
  { id: 3, question: "¿Se integra con otras herramientas?", answer: "Sí. Tenemos integración nativa con Slack, Google Workspace, GitHub, Jira, Notion y más herramientas mediante API abierta.", keywords: ["integracion", "integrar", "slack", "github", "google", "jira", "notion", "api", "herramientas"] },
  { id: 4, question: "¿Qué incluye el plan Starter?", answer: "El plan Starter incluye hasta 3 proyectos activos, tareas ilimitadas, colaboradores básicos y acceso a la app móvil. Ideal para equipos pequeños.", keywords: ["starter", "gratis", "basico", "pequeño"] },
  { id: 5, question: "¿Qué incluye el plan Pro Team?", answer: "Pro Team incluye todo lo de Starter, más proyectos ilimitados, asistente IA, permisos avanzados, reportes y soporte prioritario 24/7.", keywords: ["pro", "team", "ia", "inteligencia", "avanzado", "ilimitado"] },
  { id: 6, question: "¿Cómo empiezo a usar Project Pulse?", answer: "Crea una cuenta gratis, configura tu primer proyecto e invita a tu equipo en pocos minutos. No se requiere tarjeta de crédito.", keywords: ["empezar", "registro", "cuenta", "crear", "inicio", "comenzar"] },
  { id: 7, question: "¿Hay una prueba gratuita?", answer: "Sí. Ofrecemos 14 días de prueba gratuita con acceso completo al plan Pro Team. Sin necesidad de tarjeta de crédito.", keywords: ["prueba", "trial", "gratis", "gratuita", "free", "demo"] },
  { id: 8, question: "¿Cuánto cuesta Project Pulse?", answer: "Tenemos un plan Starter gratuito y planes de pago desde $12/mes por usuario. Consulta nuestra página de precios para más detalles.", keywords: ["precio", "costo", "cuanto", "valor", "cobrar", "pagar", "dinero", "tarifa"] },
  { id: 9, question: "¿Cuántos colaboradores puedo agregar?", answer: "En el plan Starter puedes agregar hasta 5 colaboradores. En Pro Team y Enterprise no hay límite de miembros.", keywords: ["colaboradores", "miembros", "equipo", "usuarios", "personas", "limite"] },
  { id: 10, question: "¿Tienen plan Enterprise?", answer: "Sí. El plan Enterprise incluye SSO, contrato personalizado, onboarding dedicado e integración con tus sistemas internos. Contáctanos para cotizar.", keywords: ["enterprise", "empresa", "corporativo", "grande", "sso", "contrato"] },
  { id: 11, question: "¿Cómo funciona el soporte técnico?", answer: "Ofrecemos soporte por chat y correo. Los planes Pro y Enterprise tienen soporte prioritario 24/7 con tiempo de respuesta garantizado.", keywords: ["soporte", "ayuda", "contacto", "asistencia", "tecnico", "support"] },
  { id: 12, question: "¿Puedo importar mis proyectos desde otra herramienta?", answer: "Sí. Puedes importar proyectos desde Trello, Asana, Monday, Jira y archivos CSV con nuestro importador guiado.", keywords: ["importar", "migrar", "migración", "trello", "asana", "monday", "csv", "transferir"] },
  { id: 13, question: "¿Tienen app móvil?", answer: "Sí. Contamos con apps nativas para iOS y Android, sincronizadas en tiempo real con tu cuenta en la web.", keywords: ["app", "movil", "celular", "ios", "android", "telefono", "smartphone"] },
  { id: 14, question: "¿Cómo funcionan las notificaciones?", answer: "Las notificaciones son inteligentes: solo te avisan cuando hay algo relevante para ti, sin spam. Puedes personalizar cuáles recibir por proyecto.", keywords: ["notificaciones", "avisos", "alertas", "spam", "configurar"] },
  { id: 15, question: "¿Puedo usar Project Pulse en español?", answer: "Sí. Project Pulse está disponible completamente en español, inglés y portugués. Puedes cambiar el idioma desde tu perfil.", keywords: ["idioma", "español", "lenguaje", "ingles", "portugues"] },
  { id: 16, question: "¿Qué pasa con mis datos si cancelo?", answer: "Tienes 30 días para exportar todos tus datos después de cancelar. Después de ese período los datos se eliminan de forma segura.", keywords: ["datos", "exportar", "eliminar", "borrar", "cancelar", "baja"] },
  { id: 17, question: "¿Ofrecen reembolsos?", answer: "Si no estás satisfecho en los primeros 30 días, te devolvemos el dinero sin preguntas.", keywords: ["reembolso", "devolucion", "garantia", "dinero", "satisfaccion"] },
  { id: 18, question: "¿Cómo funciona el asistente IA?", answer: "El asistente IA te ayuda a redactar tareas, resumir hilos de comentarios, detectar bloqueos en proyectos y sugerir prioridades automáticamente.", keywords: ["ia", "inteligencia", "artificial", "asistente", "ai", "automatico", "gpt"] },
];
...
const painPoints = [
  "Buscando en hilos de Slack interminables de hace 3 días.",
  "Hojas de Excel llamadas 'Presupuesto_V3_FINAL.xlsx'.",
  "Reuniones de sincronización de 1 hora que podrían ser un mail."
];

const pulseBenefits = [
  "Una única fuente de verdad. Si está en Pulse, es la actual.",
  "Notificaciones inteligentes que solo avisan cuando es necesario.",
  "Claridad absoluta sobre quién hace qué y para cuándo."
];

// --- CUSTOM HOOK PARA EL CARRUSEL ---
function useCarousel(length: number, delay: number = 6000) {
...
export default function HomePage() {
  ...
  return (
    <div className="relative bg-[#fafafa] font-sans antialiased text-slate-900 selection:bg-indigo-200 selection:text-indigo-900">
      ...
      {/* Nota: Para producción, es recomendable mover estos keyframes 
        a tu archivo globals.css o extender el tailwind.config.ts 
      */}
        ...
        >
          <div className="absolute inset-0 z-0 bg-slate-950">
            {images.map((image, index) => (
              <div
                key={index}
                className={`absolute inset-0 transition-opacity duration-1000 ease-in-out ${index === carousel.currentSlide ? "opacity-100 z-10" : "opacity-0 z-0"
                  }`}
                aria-hidden={index !== carousel.currentSlide}
              >
                <Image
                  src={image}
                  alt={`Fondo interactivo del hero ${index + 1}`}
                  fill
                  className={`object-cover transition-transform duration-[10000ms] ease-linear ${index === carousel.currentSlide ? "scale-110" : "scale-100"
                    }`}
                  priority={index === 0}
                />
          ...
          <div className="absolute bottom-32 sm:bottom-12 left-1/2 -translate-x-1/2 z-30 flex items-center gap-3">
            {images.map((_, index) => (
              <button
                ...
                aria-current={index === carousel.currentSlide}
                className={`h-1.5 rounded-full transition-all duration-500 focus:outline-none focus:ring-2 focus:ring-white/50 ${index === carousel.currentSlide ? "w-8 bg-white" : "w-2 bg-white/30 hover:bg-white/50"
                  }`}
              />
            ))}
          </div>
          ...
          <button
            ...
            className="absolute left-4 sm:left-8 top-1/2 -translate-y-1/2 z-30 p-3 rounded-full text-white/50 hover:text-white hover:bg-white/10 opacity-0 group-hover:opacity-100 transition-all backdrop-blur-md border border-transparent hover:border-white/10 focus:outline-none focus:ring-2 focus:ring-white/50"
          >
            <ChevronLeft size={28} />
          </button>
          ...
          <button
            ...
            className="absolute right-4 sm:right-8 top-1/2 -translate-y-1/2 z-30 p-3 rounded-full text-white/50 hover:text-white hover:bg-white/10 opacity-0 group-hover:opacity-100 transition-all backdrop-blur-md border border-transparent hover:border-white/10 focus:outline-none focus:ring-2 focus:ring-white/50"
          >
          ...
          <div className="relative z-20 w-full max-w-5xl px-4 sm:px-6 text-center text-white mt-16">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <button
                ...
                className="group h-14 w-full sm:w-auto px-8 bg-white hover:bg-slate-50 text-slate-900 font-bold rounded-full transition-all duration-300 hover:scale-105 shadow-[0_0_40px_rgba(255,255,255,0.3)] flex items-center justify-center gap-2"
              >
                Empezar Gratis
              ...
              <button
                ...
                className="h-14 w-full sm:w-auto px-8 bg-white/5 border border-white/10 backdrop-blur-md text-white font-semibold rounded-full hover:bg-white/10 transition-all flex items-center justify-center"
              >
          ...
          <div className="max-w-5xl mx-auto px-4 sm:px-6 text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto items-center">
              ...
              <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-xl shadow-slate-200/20 text-left">
                ...
                <button className="w-full py-4 rounded-full border-2 border-slate-200 text-slate-700 font-bold hover:border-slate-300 hover:bg-slate-50 transition-all focus:outline-none focus:ring-2 focus:ring-slate-300">
                  Empezar con Starter
                </button>
              </div>
              ...
              <div className="relative p-[2px] rounded-[2.5rem] bg-gradient-to-br from-indigo-500 via-purple-500 to-blue-500 shadow-2xl shadow-indigo-500/20 hover:shadow-indigo-500/40 transition-shadow duration-500">
                ...
                <div className="bg-slate-950 p-10 rounded-[calc(2.5rem-2px)] text-left h-full relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    ...
                    <button className="w-full py-4 rounded-full bg-white text-slate-900 font-bold hover:bg-slate-100 transition-transform hover:scale-[1.02] active:scale-95 focus:outline-none focus:ring-2 focus:ring-indigo-400">
            ...
            <div className="space-y-4">
              {faqEntries.slice(0, 4).map((faq) => (
                <details key={faq.id} className="group bg-white rounded-2xl border border-slate-200 hover:border-slate-300 transition-colors [&_summary::-webkit-details-marker]:hidden shadow-sm">
                  <summary className="flex justify-between items-center gap-4 font-bold cursor-pointer p-6 text-slate-900 text-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:rounded-2xl">
                    {faq.question}
          ...
          <div className="bg-[#09090b] rounded-[3rem] p-12 md:p-24 text-center text-white relative overflow-hidden shadow-2xl">
            ...
            <div className="relative z-10 max-w-3xl mx-auto">
              ...
              <div className="flex flex-col sm:flex-row justify-center gap-4">
                <button
                  onClick={() => router.push("/register")}
                  className="bg-white text-slate-900 px-8 py-4 rounded-full font-bold text-lg hover:scale-105 transition-all shadow-[0_0_30px_rgba(255,255,255,0.2)] focus:outline-none focus:ring-2 focus:ring-white/50"
                >
                  Crear cuenta gratis
                </button>
                <button
                  onClick={() => router.push("/ventas")}
                  className="bg-white/10 backdrop-blur-md border border-white/10 text-white px-8 py-4 rounded-full font-bold text-lg hover:bg-white/20 transition-all focus:outline-none focus:ring-2 focus:ring-white/50"
                >
                  Hablar con ventas
                </button>
              </div>
